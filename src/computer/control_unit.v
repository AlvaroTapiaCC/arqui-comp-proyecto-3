...
module control_unit(
  ...
  // Flags latcheadas (para saltos condicionales)
  input wire       zf,
  input wire       nf,
  input wire       cf,
  input wire       vf,
  ...
  // Extensiones para acceso a memoria / write-back múltiple
  ...
  // Extensiones para CMP y saltos
  output reg       latch_flags, // fuerza latch de flags sin escribir registros (CMP)
  output reg       branch_taken // salto tomado
);

  // ===== Enumeración (limpia) de extensiones futuras =====
  // (Placeholder) Cuando se integre direccionamiento a memoria y saltos según tabla del profesor,
  // se definirán aquí localparams secuenciales y coherentes.
  ...
  always @* begin
    ...
    alu_op = 4'h0;            
    sel_data = 2'b00; // ALU por defecto
  we_mem = 1'b0;    // sin escritura memoria por defecto
  addr_sel = 1'b0;  // modo directo (imm) por defecto
  latch_flags = 1'b0;
  branch_taken = 1'b0;
